...
VelixVault implements zero-knowledge encryption where sensitive user data never leaves the client device in unencrypted form. Below is a detailed explanation of how the system secures user data and communications.

**What is Zero-Knowledge in VelixVault**:
...
## Diagram
The security architecture diagram below illustrates all encryption and authentication processes used in VelixVault to secure user data and communications.

<ThemedImage
  alt="VelixVault Security Architecture Diagram"
  sources={{
...
You can also view the diagram in a browser-friendly HTML format: [VelixVault Security Architecture](pathname:///assets/diagrams/security-architecture/aliasvault-security-architecture.html)
...
VelixVault includes a virtual passkey authenticator that implements the WebAuthn Level 2 specification, allowing users to securely store and use passkeys for passwordless authentication across websites and services.
...
2. Authenticator Data
    - Uses VelixVault's unique AAGUID (Authenticator Attestation GUID): `a11a5faa-9f32-4b8c-8c5d-2f7d13e8c942`
...
VelixVault includes a secure "Login with Mobile" feature that allows users to unlock their vault on web browsers or browser extensions by scanning a QR code with their mobile app. This system provides a convenient authentication method while maintaining zero-knowledge security through end-to-end encryption.
