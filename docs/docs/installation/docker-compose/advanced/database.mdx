...
This page explains how to import/export on the VVault server database when installed via the all-in-one Docker container, and how to point VVault at an external PostgreSQL server.

<TOCInline toc={toc} />

## Database Export
In order to backup the VVault server database (which includes all encrypted user vaults as well), you can use the following command. Run this command from the directory where your VVault `docker-compose.yml` is located.
...
To restore a previously exported database, you can use the following snippet. Run these commands from the directory where your VVault `docker-compose.yml` is located.
