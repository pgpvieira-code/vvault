...
# Database (install.sh)
This page explains how to import/export on the VelixVault server database via the `./install.sh` script, and how to point VelixVault at an external PostgreSQL server.

<TOCInline toc={toc} />

## Database Export
In order to backup the VelixVault server database (which includes all encrypted user vaults as well), you can use the `install.sh` script. This streams a compressed export of the database to the file you redirect to, while the services keep running. This script has to be run in a root terminal, not using sudo.
...
By default VelixVault runs a bundled PostgreSQL container and no database configuration is needed. If you prefer to use your own PostgreSQL server instead (e.g. a managed instance or a dedicated database host):
...
3. Restart VelixVault: `./install.sh restart`.
