...
---
# Updating to v0.23.0
Since v0.23.0, VVault has moved from `lanedirt/aliasvault` to the new `aliasvault/aliasvault` GitHub organization, and Docker image names have been simplified.

## Update Methods

### 1. Installed via install.sh

If you have installed VVault using the official `install.sh` method, you don't need to do anything. Running the built-in `install.sh update` command will take care of all necessary changes for you.
...
If you have manually installed VVault via a custom `docker-compose.yml` file or other Docker management interface such as Portainer, you need to update your Docker image references.
